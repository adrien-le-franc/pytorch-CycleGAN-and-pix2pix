...
class Pix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(2*opt.input_nc, opt.output_nc, opt.ngf,
                                      opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids)
        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD = networks.define_D(2*opt.input_nc + opt.output_nc, opt.ndf,
        ...
        if self.isTrain:
            self.fake_ABC_pool = ImagePool(opt.pool_size)
            self.old_lr = opt.lr
    ...
    def set_input(self, input):
        AtoB = self.opt.which_direction == 'AtoB' ### change ? remove ?
        ...
        self.image_paths = input['A_paths' if AtoB else 'B_paths'] ### always the same ?

    def forward(self):
        self.real_A = Variable(self.input_A)
        self.real_C = Variable(self.input_C)
        AC = torch.cat((self.real_A, self.real_C), 1)
        self.fake_B = self.netG(AC)
    ...
    def backward_D(self):
        ...
        fake_ABC = self.fake_ABC_pool.query(torch.cat((self.real_A, self.fake_B, self.real_C), 1).data)
        pred_fake = self.netD(fake_ABC.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False)

        # Real
        real_ABC = torch.cat((self.real_A, self.real_B, self.real_C), 1)
    ...
    def backward_G(self):
        ...
        fake_ABC = torch.cat((self.real_A, self.fake_B, self.real_C), 1)
